...
class SentenceSimilarity():
    ...
    # 构建其他复杂模型前需要的简单模型
    def simple_model(self,min_frequency = cfg.get('model','min_frequency')):
    ...
    # 求最相似的句子
    def similarity(self,sentence,num ):
        sentence_vec = self.sentence2vec(sentence)
        sims = self.index[sentence_vec]
        lsit = sims.tolist()
        max_num_index_list = map(lsit.index, heapq.nlargest(num, lsit))
